...
- `src/Components/System` は最低レイヤーです。ButtonやIconなどの単体部品のみ置くこと。
- `src/Components/Patterns` は中レイヤーです。`System` を組み合わせた再利用部品のみ置くこと。
- `playground` は利用側です。実際のページ組み立てやモックデータはここに置くこと。
- ページレイアウトそのものを `src` に置いてはならないです。`src` は部品提供だけを担当すること。
